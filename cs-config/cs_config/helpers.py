...
def random_seed(user_mods, year):
    ...
    def random_seed_from_subdict(subdict):
        ...
        txt = "".join(all_vals).encode("utf-8")
...
def fuzzed(df1, df2, reform_affected, table_row_type):
    ...
    if table_row_type == "xbin":
        df1 = add_income_table_row_variable(
            df1, "expanded_income", STANDARD_INCOME_BINS
        )
        df2["expanded_income_baseline"] = df1["expanded_income"]
        df2 = add_income_table_row_variable(
            df2, "expanded_income_baseline", STANDARD_INCOME_BINS
        )
        del df2["expanded_income_baseline"]
    elif table_row_type == "xdec":
        df1 = add_quantile_table_row_variable(
            df1, "expanded_income", 10, decile_details=True
        )
        df2["expanded_income_baseline"] = df1["expanded_income"]
        df2 = add_quantile_table_row_variable(
            df2, "expanded_income_baseline", 10, decile_details=True
        )
        del df2["expanded_income_baseline"]
...
def nth_year_results(tb, year, user_mods, fuzz, return_html=True):
    ...
    if return_html:
        res = {}
        for id in sres:
            res[id] = [{"dimension": year, "raw": sres[id]}]
...
def postprocess(data_to_process):
    ...
    """
    labels = {
        x: DIFF_TABLE_LABELS[i] for i, x in enumerate(DIFF_TABLE_COLUMNS)
    }
    labels.update(
        {x: DIST_TABLE_LABELS[i] for i, x in enumerate(DIST_TABLE_COLUMNS)}
    )

    # nested functions used below
    def label_columns(pdf):
        pdf.columns = [
            (labels[str(col)] if str(col) in labels else str(col))
    ...
    def append_year(pdf, year):
        ...
        """
        pdf.columns = ["{}_{}".format(col, year) for col in pdf.columns]
        return pdf
    ...
    formatted = {
        "tbl_outputs": arbitrary_defaultdict(),
    ...
    for id, pdfs in data_to_process.items():
        if id.startswith("aggr"):
            ...
            tbl = pdfs[0]["raw"]
            tbl.index = pd.Index(
                RESULTS_TOTAL_ROW_KEY_LABELS[i] for i in tbl.index
            )
            # format table
            ...
            for i in pdfs:
                ...
                title = "{} ({})".format(RESULTS_TABLE_TITLES[id], year)
                # format table
                for col in tbl.columns:
